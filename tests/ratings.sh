  ...
  do
    ...
    get() {
      get_all() {
        ...
        printf "\r\n"
        curl -i -X GET "http://localhost:5000/api/ratings" -H  "accept: application/json"
        printf "\r\n\r\n"
      }
      if [[ $rqst == '1' ]]; then
        get_all
      ...
      get_single() {
        ...
        printf "\r\n"
        curl -i -X GET "http://localhost:5000/api/ratings/$id" -H  "accept: application/json"
        printf "\r\n\r\n"
      }
      if [[ $rqst == '2' ]]; then
        get_single
    ...
    post() {
      ...
      printf "\r\n"
      curl -i -X POST "http://localhost:5000/api/ratings" -H  "accept: application/json" -H  "Content-Type: application/json" -d "$data"
      printf "\r\n\r\n"
    }
    if [[ $rqst == '3' ]]; then
      post
    ...
    delete() {
      delete_single() {
        ...
        read -p "Enter Rating ID: " id
        printf "\r\n"
        curl -i -X DELETE "http://localhost:5000/api/ratings/$id" -H  "accept: application/json"
        printf "\r\n\r\n"
      }
      if [[ $rqst == '4' ]]; then
        delete_single
      ...
      delete_all() {
        ...
        printf "\r\n"
        curl -i -X DELETE "http://localhost:5000/api/ratings" -H  "accept: application/json"
        printf "\r\n\r\n"
      }
      if [[ $rqst == '5' ]]; then
        delete_all
    ...
    head() {
      head_all() {
        ...
        printf "\r\n"
        curl -i --head "http://localhost:5000/api/ratings/$id" -H  "accept: application/json"
        printf "\r\n\r\n"
      }
      if [[ $rqst == '6' ]]; then
        head_all
